...
/**
 * Submit a memory version's zip archive to VirusTotal for scanning.
 * Skips the scan when every file is detected as text.
 * Marks as rate_limited if VT quota is exhausted.
 */
export const submitMemoryScan = internalAction({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    const files = await ctx.runQuery(internal.virusTotalScan.getMemoryVersionFiles, {
      versionId: args.versionId,
    });
    ...
    try {
      ...
      await ctx.scheduler.runAfter(60_000, internal.virusTotalScanActions.pollMemoryResult, {
        versionId: args.versionId,
...
/**
 * Poll VirusTotal for memory version analysis results.
 */
export const pollMemoryResult = internalAction({
  args: {
    versionId: v.id("memoryVersions"),
    memoryId: v.id("memories"),
    analysisId: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (result.status === "queued") {
        const delay = Math.min(60_000 * 1.5 ** args.attemptNumber, 300_000);
        await ctx.scheduler.runAfter(delay, internal.virusTotalScanActions.pollMemoryResult, {
          ...args,
          attemptNumber: args.attemptNumber + 1,
        });
        return;
      ...
      if (positives > 0) {
        await ctx.runMutation(internal.virusTotalScan.flagMemory, {
          memoryId: args.memoryId,
          versionId: args.versionId,
          scanResult,
        });
      ...
      if (args.attemptNumber < MAX_POLL_ATTEMPTS - 1) {
        await ctx.scheduler.runAfter(
          60_000,
          internal.virusTotalScanActions.pollMemoryResult,
          { ...args, attemptNumber: args.attemptNumber + 1 },
        );
      } else {
        await ctx.runMutation(internal.virusTotalScan.setMemoryScanStatus, {

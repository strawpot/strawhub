...
function MemoryDetailPage() {
  ...
  const memory = useQuery(api.memories.getBySlug, { slug });

  useSEO({
    title: memory ? `${memory.displayName} - StrawHub` : "StrawHub",
  ...
  const starred = useQuery(
    api.stars.isStarred,
    memory ? { targetId: memory._id } : "skip",
  );
  ...
  const isOwner = !!(currentUser && memory && memory.ownerUserId === currentUser._id);
  ...
  if (memory === null) {
    return (
      <div className="space-y-4">
        <Link to="/memories" className="text-sm text-gray-400 hover:text-white">
          &larr; Back to Memories
        </Link>
        <p className="text-gray-400">Memory not found.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            {memory.latestVersion && (
              <span className="rounded bg-orange-500/10 border border-orange-500/30 px-2.5 py-0.5 text-sm font-semibold text-orange-400 font-mono">
                v{memory.latestVersion.version}
              </span>
            )}
          </div>
          <p className="text-sm text-gray-500 font-mono mt-1">/{memory.slug}</p>
        </div>

        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {isOwner && (
            <button
              onClick={() => navigate({ to: "/upload", search: { updateSlug: memory.slug, kind: "memory" } })}
          ...
          {memory.latestVersion && memory.zipUrl && (
            <button
              onClick={async () => {
                ...
                a.href = url;
                a.download = `${memory.slug}-v${memory.latestVersion!.version}.zip`;
                a.click();
      ...
      {memory.owner && (
        <div className="flex items-center gap-3">
          ...
          {memory.owner.image ? (
            <img
              src={memory.owner.image}
              alt={memory.owner.displayName ?? memory.owner.handle ?? ""}
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-200">
              {memory.owner.displayName ?? memory.owner.handle ?? "unknown"}
            </span>
            {memory.owner.handle && (
              <span className="text-gray-500">@{memory.owner.handle}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-6 text-sm text-gray-400">
        <button
          onClick={() => {
            if (currentUser) {
              toggleStar({ targetId: memory._id, targetKind: "memory" });
            }
          }}
        ...
        {memory.latestVersion && (
          <span>{memory.latestVersion.downloads ?? 0} current installs</span>
        )}
        <span>{memory.stats.downloads} all-time installs</span>
        <span>{memory.stats.versions} versions</span>
      </div>
      ...
      {showReportForm && (
        ...
        >
          <div className="w-full max-w-md rounded-lg border border-gray-700 bg-gray-900 p-5 shadow-xl space-y-4">
            <h3 className="text-base font-semibold text-white">Report memory</h3>
            ...
            <div className="space-y-2">
              <textarea
                ...
                placeholder="What should moderators know about this memory?"
                maxLength={1000}
                autoFocus
      ...
      {memory.summary && <p className="text-gray-300">{memory.summary}</p>}

      {/* Tabbed: Files / Versions */}
      <MemoryDetailTabs
        files={memory.filesWithUrls}
        versions={versions ?? []}
      ...
      <CommentsSection
        targetId={memory._id}
        targetKind="memory"
        commentCount={memory.stats.comments}
...
function MemoryDetailTabs({
  ...
  const otherFiles = files.filter((f) => f.path !== "MEMORY.md");
    ...
    <div className="rounded-lg border border-gray-800 overflow-hidden">
      ...
      {tab === "files" && (
        <div className="p-4 space-y-4">
          {/* MEMORY.md always shown at top */}
          {memoryMdFile && <MemoryMdViewer file={memoryMdFile} />}

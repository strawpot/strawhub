...
function IntegrationDetailPage() {
  ...
  const integration = useQuery(api.integrations.getBySlug, { slug });

  useSEO({
    title: integration ? `${integration.displayName} - StrawHub` : "StrawHub",
    description: integration?.summary || undefined,
    url: `/integrations/${slug}`,
  });
  const { results: versions, status: versionsStatus, loadMore: loadMoreVersions } = usePaginatedQuery(
    api.integrations.getVersions,
    integration ? { integrationId: integration._id } : "skip",
    { initialNumItems: 20 },
  );
  ...
  const starred = useQuery(
    api.stars.isStarred,
    integration ? { targetId: integration._id } : "skip",
  );
  ...
  const isOwner = !!(currentUser && integration && integration.ownerUserId === currentUser._id);
  ...
  if (integration === null) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-gray-400">Integration not found.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            {integration.latestVersion && (
              <span className="rounded bg-orange-500/10 border border-orange-500/30 px-2.5 py-0.5 text-sm font-semibold text-orange-400 font-mono">
                v{integration.latestVersion.version}
              </span>
            )}
          </div>
          <p className="text-sm text-gray-500 font-mono mt-1">/{integration.slug}</p>
        </div>

        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {isOwner && (
            <button
              onClick={() => navigate({ to: "/upload", search: { updateSlug: integration.slug, kind: "integration" } })}
          ...
          {integration.latestVersion && integration.zipUrl && (
            <button
              onClick={async () => {
                trackDownload({ targetKind: "integration", slug: integration.slug, version: integration.latestVersion!.version, userId: currentUser?._id });
                const res = await fetch(integration.zipUrl!);
                const blob = await res.blob();
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                a.download = `${integration.slug}-v${integration.latestVersion!.version}.zip`;
                a.click();
      ...
      {integration.owner && (
        <div className="flex items-center gap-3">
          ...
          {integration.owner.image ? (
            <img
              src={integration.owner.image}
              alt={integration.owner.displayName ?? integration.owner.handle ?? ""}
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-200">
              {integration.owner.displayName ?? integration.owner.handle ?? "unknown"}
            </span>
            {integration.owner.handle && (
              <span className="text-gray-500">@{integration.owner.handle}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-6 text-sm text-gray-400">
        <button
          onClick={() => {
            if (currentUser) {
              toggleStar({ targetId: integration._id, targetKind: "integration" });
            }
          }}
        ...
        {integration.latestVersion && (
          <span>{integration.latestVersion.downloads ?? 0} current installs</span>
        )}
        <span>{integration.stats.downloads} all-time installs</span>
        <span>{integration.stats.versions} versions</span>
      </div>
      ...
      {showReportForm && (
        ...
        >
          <div className="w-full max-w-md rounded-lg border border-gray-700 bg-gray-900 p-5 shadow-xl space-y-4">
            <h3 className="text-base font-semibold text-white">Report integration</h3>
            ...
            <div className="space-y-2">
              <textarea
                ...
                placeholder="What should moderators know about this integration?"
                maxLength={1000}
                autoFocus
      ...
      {integration.summary && <p className="text-gray-300">{integration.summary}</p>}

      {/* Tabbed: Files / Versions */}
      <IntegrationDetailTabs
        files={integration.filesWithUrls}
        versions={versions ?? []}
        versionsStatus={versionsStatus}
        loadMoreVersions={loadMoreVersions}
        latestVersionId={integration.latestVersionId}
        slug={integration.slug}
      ...
      <CommentsSection
        targetId={integration._id}
        targetKind="integration"
        commentCount={integration.stats.comments}
...
function IntegrationDetailTabs({
  files,
  versions,
  versionsStatus,
  loadMoreVersions,
  ...
  slug: string;
  trackDownload: (args: { targetKind: "integration"; slug: string; version?: string; userId?: string }) => void;
  userId?: string;
}) {
  const [tab, setTab] = useState<"files" | "versions">("files");

  const integrationMdFile = files.find((f) => f.path === "INTEGRATION.md");
  const otherFiles = files.filter((f) => f.path !== "INTEGRATION.md");
    ...
    <div className="rounded-lg border border-gray-800 overflow-hidden">
      ...
      {tab === "files" && (
        <div className="p-4 space-y-4">
          {integrationMdFile && <IntegrationMdViewer file={integrationMdFile} />}
            ...
            <div key={ver._id} className="px-4 py-3 space-y-1">
              <div className="flex items-center gap-2">
                ...
                {ver.zipUrl && (
                  <button
                    onClick={async () => {
                      trackDownload({ targetKind: "integration", slug, version: ver.version, userId });

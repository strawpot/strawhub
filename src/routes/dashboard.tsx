...
function UserContent({ userId }: { userId: string; handle?: string }) {
  ...
  const { results: memories, status: memoriesStatus, loadMore: loadMoreMemories } = usePaginatedQuery(
    api.memories.listByOwner,
    { userId: userId as any },
    { initialNumItems: 50 },
  );
            ...
            {agents.map((a) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    to="/upload"
                    search={{ updateSlug: a.slug, kind: "agent" }}
                    className="rounded border border-gray-700 px-3 py-1.5 text-xs text-gray-300 hover:bg-gray-800"
                  >
                    New Version
                  </Link>
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-gray-500">
                    ...
                    <span>{m.stats.versions} versions</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    to="/upload"
                    search={{ updateSlug: m.slug, kind: "memory" }}
                    className="rounded border border-gray-700 px-3 py-1.5 text-xs text-gray-300 hover:bg-gray-800"
                  >
                    New Version
                  </Link>

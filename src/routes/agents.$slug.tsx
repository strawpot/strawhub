...
function AgentDetailPage() {
  ...
  useSEO({
    title: agent ? `${agent.displayName} - StrawHub` : "StrawHub",
  ...
  const isOwner = !!(currentUser && agent && agent.ownerUserId === currentUser._id);
  ...
  if (agent === null) {
    return (
      <div className="space-y-4">
        <Link to="/agents" className="text-sm text-gray-400 hover:text-white">
          &larr; Back to Agents
        </Link>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 font-mono mt-1">/{agent.slug}</p>
        </div>

        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {isOwner && (
            <button
              onClick={() => navigate({ to: "/upload", search: { updateSlug: agent.slug, kind: "agent" } })}
          ...
          {agent.latestVersion && agent.zipUrl && (
            <button
              onClick={async () => {
                ...
                a.href = url;
                a.download = `${agent.slug}-v${agent.latestVersion!.version}.zip`;
                a.click();
        ...
        <div className="flex items-center gap-3">
          ...
          {agent.owner.image ? (
            <img
              ...
              alt={agent.owner.displayName ?? agent.owner.handle ?? ""}
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-200">
              {agent.owner.displayName ?? agent.owner.handle ?? "unknown"}
            </span>
            {agent.owner.handle && (
              <span className="text-gray-500">@{agent.owner.handle}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-6 text-sm text-gray-400">
        <button
          onClick={() => {
            if (currentUser) {
              toggleStar({ targetId: agent._id, targetKind: "agent" });
            }
          }}
        ...
        {agent.latestVersion && (
          <span>{agent.latestVersion.downloads ?? 0} current installs</span>
        )}
        <span>{agent.stats.downloads} all-time installs</span>
        <span>{agent.stats.versions} versions</span>
      </div>
      ...
      {showReportForm && (
        ...
        >
          <div className="w-full max-w-md rounded-lg border border-gray-700 bg-gray-900 p-5 shadow-xl space-y-4">
            <h3 className="text-base font-semibold text-white">Report agent</h3>
            ...
            <div className="space-y-2">
              <textarea
                ...
                placeholder="What should moderators know about this agent?"
                maxLength={1000}
                autoFocus
      ...
      {agent.summary && <p className="text-gray-300">{agent.summary}</p>}
      ...
      <CommentsSection
        targetId={agent._id}
        targetKind="agent"
        commentCount={agent.stats.comments}
...
function AgentDetailTabs({
  ...
  const otherFiles = files.filter((f) => f.path !== "AGENT.md");
    ...
    <div className="rounded-lg border border-gray-800 overflow-hidden">
      ...
      {tab === "files" && (
        <div className="p-4 space-y-4">
          {/* AGENT.md always shown at top */}
          {agentMdFile && <AgentMdViewer file={agentMdFile} />}

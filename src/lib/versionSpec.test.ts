...
describe("satisfiesVersion", () => {
  ...
  it("^ caret: same major and >=", () => {
    ...
    expect(satisfiesVersion("2.0.0", spec)).toBe(false);
    expect(satisfiesVersion("1.1.0", spec)).toBe(false);
    expect(satisfiesVersion("0.2.0", spec)).toBe(false);
  });
});
...
describe("splitDependencies", () => {
  ...
  it("splits role-only deps", () => {
    expect(
      splitDependencies(["role:implementer", "role:reviewer>=2.0.0"]),
    ).toEqual({
  ...
  it("trims whitespace", () => {
    expect(
      splitDependencies(["  code-review  ", "  role:implementer  "]),
    ).toEqual({
      skills: ["code-review"],
      roles: ["implementer"],
    });

...
test.describe("Smoke tests", () => {
  ...
  test("upload page requires authentication", async ({ page }) => {
    ...
    await expect(page.locator("text=Sign in with GitHub to publish roles, skills, agents, memories, and integrations.")).toBeVisible();
  });
  ...
  test("homepage renders hero content", async ({ page }) => {
    ...
    await expect(page.locator("text=strawhub install role pr-reviewer")).toBeVisible();
  });

  test("search page loads with input", async ({ page }) => {
    await page.goto("/search");
    await expect(page.locator("h1")).toContainText("Search");
    await expect(page.locator('input[placeholder="Search roles, skills, agents, memories, and integrations..."]')).toBeVisible();
  ...
  test("dashboard requires authentication", async ({ page }) => {
    ...
    await expect(page.locator("text=Sign in with GitHub to manage your published roles, skills, agents, memories, and integrations.")).toBeVisible();
  });
  ...
  test("stars page requires authentication", async ({ page }) => {
    ...
    await expect(page.locator("text=Sign in with GitHub to see roles, skills, agents, memories, and integrations you've starred.")).toBeVisible();
  });
